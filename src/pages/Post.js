...
const Post = () => {
  ...
  const postComment = async () => {
    ...
    const API_URL = 'http://localhost:5139/api/comment'
    let result = await axios({
        headers: {
  ...
  const ratePost = async () => {
      if(post.userStar) {
        ...
        const API_URL = 'http://localhost:5139/api/star'
        let result = await axios({
            headers: {
              'Authorization': `Bearer ${user.token}`
            },
    ...
    <Layout>
      {
        ...
        /> :
          <div className='w-full lg:w-4/6 flex flex-col justify-center items-center'>
            <div style={{ backgroundImage: `url(${post.imageUrl})` }} className={'relative w-full h-80 bg-cover'}>
            ...
            <div  className='w-28 h-28 rounded-full border-2 border-violet-500 bg-cover'
            ...
            <h2 className='text-xl font-bold dark:text-white'>{post.user.username}</h2>
               ...
               <div className='flex items-center'>
                {
                  starArr.map((star, index) => {
                    if(star <= starRating) {
                      return (
                        <AiFillStar className='text-yellow-400 cursor-pointer' id={star} key={index} size={32} onClick={() =>setStarFunc(star)}/>
                      )
                    }
                    return (
                      <AiOutlineStar className='cursor-pointer' id={star} key={index} size={32} onClick={() =>setStarFunc(star)}/>
                    )
                  })
                }
                
                <div className='mx-4'>
                <button onClick={ratePost} className='btn-solid-no-animation'>Rate</button>
               </div>
               </div>
            </div>
            }
            ...
            <div className='w-full flex flex-col justify-center items-start bg-gray-300 px-6 py-2 space-y-2'>
               {
                ...
                <h3 className='text-lg font-bold'>Leave a comment</h3>
                  <textarea onChange={(e) => setUserComment(e.target.value)} value={userComment} className='w-full h-32 bg-slate-200' placeholder='Share your thoughts...'></textarea>
                  <div className='flex flex-row items-center space-x-2'>
                    <div>
                      <button onClick={postComment} disabled={false} className='btn-solid-no-animation'>Comment</button>
                    </div>
        
                    <span className='font-bold text-base'>Anonymous mode</span>
                    <input onChange={()=> setAnonyMode(prevState => !prevState)} type={'checkbox'}></input>
                </div>  
                </> :
                <div className='w-full h-44 flex flex-col justify-center items-center space-y-2'>
                  <AiFillLock size={60} className="text-gray-700"/>
                    <h2 className='text-lg'>You need to be logged in to comment and rate</h2>
            ...
            <div className='w-full space-y-2 bg-gray-300'>
              {
                post.comments.map(comment => {
                  return (
                    <div key={comment.id} className='w-full  bg-slate-200 p-2 space-y-2'>
                      <div className='flex space-x-1'>
                        <h3 className='text-lg text-violet-500'>{comment.anonymous ? "Anonymous" : comment.userInfo.username}</h3>
                        <span>posted</span>
