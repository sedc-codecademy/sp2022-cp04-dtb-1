...
import {FormControl , InputLabel, OutlinedInput, MenuItem, ListItemText, Checkbox, Select } from '@mui/material'
...
const Create = () => {
    ...
    useEffect(()=> {
        if(postForm.imageUrl) {
              setPreview(<img src={postForm.imageUrl} className='post-background-image-img' alt={'cover'}></img>);
        } else {
            setPreview(null);
        }
    }, [postForm.imageUrl])
    ...
    const sendNewsletter = async (postDto) => {
      ...
      try {
        let result = await axios({
          method: 'post',
          url: API_URL + `/newsletter?postTitle=${postDto.title}&postId=${postDto.id}`,
        });
            ...
            <form onSubmit={onSubmit} className="bg-slate-200 dark:bg-gray-600">
                <div className="post-card-create group" >
                    ...
                    <div className='bg-white dark:bg-gray-600 flex items-center justify-start p-2'>
                      <input type="url" name="imageUrl" onChange={onChange} value={postForm.imageUrl} placeholder={"image url..."} className='bg-slate-100 dark:bg-slate-500 shadow-md w-full'></input>
                    </div>
                      
                    <div className='post-data-create space-y-3'>
                        <h1 className='post-title'>
                            <input type="text" name="title" className='bg-slate-100 dark:bg-slate-500 shadow-md w-full' value={postForm.title} onChange={onChange} placeholder="Title"></input>  
                        </h1>
                        <div className='post-description-create'>
                              <textarea name="description" className='w-full h-full resize-none bg-slate-100 dark:bg-slate-500 shadow-md' maxLength="200" value={postForm.description} onChange={onChange} placeholder='Description'></textarea> 
                        </div>
                        <div className='post-description-create'>
                              <textarea name="body" className='w-full h-full resize-none bg-slate-100 dark:bg-slate-500 shadow-md' maxLength="1000" value={postForm.body} onChange={onChange} placeholder='Body'></textarea> 
                        </div>
                    </div>
                </div>
