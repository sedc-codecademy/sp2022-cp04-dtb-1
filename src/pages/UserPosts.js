          ...
          <div className='w-full lg:w-3/6 p-6 mx-auto space-y-2'>
            {posts && 
              posts.map((post, index) => {
                return (
                  <div key={index} className='flex h-24 justify-between items-center shadow-md p-2 dark:bg-gray-500 dark:text-white'>
                      <h2>{post.title}</h2>
                      <div className='flex'>
                        ...
                        <MdDelete className='cursor-pointer hover:text-red-500'  size="30" onClick={() => deletePost(post.id)}/>
                      </div>
